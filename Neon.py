...
bot_token = input(Colorate.Horizontal(Colors.blue_to_purple, "Enter Bot Token ~ "))
server_id = input(Colorate.Horizontal(Colors.blue_to_purple, "Enter Guild id ~ "))
...
async def show_menu():
    while True:
        menu_text = f"""
                      {p}[{w}1{p}] AUTO FUCK       {p}[{w}6{p}] GET PRMS 
                      {p}[{w}2{p}] WEBHOOK SPAM    {p}[{w}7{p}] DM All
                      {p}[{w}3{p}] Create Roles    {p}[{w}8{p}] Create Channel
                      {p}[{w}4{p}] Delete Roles    {p}[{w}9{p}] Delete Channel
                      {p}[{w}5{p}] FUCK ALL        {p}[{w}10{p}] change_server
                      
                                 {p}[{w}11{p}] Close
        """
        print(Center.XCenter(menu_text))
        
        choice = await bot.loop.run_in_executor(None, input, Colorate.Horizontal(Colors.blue_to_purple, "                       Neon Input ~ "))
        ...
        if choice == '1':
            ...
            print(Colorate.Horizontal(Colors.blue_to_purple, "Please pick a valid choice"))
            continue
...
[+] Command Used: Create Channels - {channels_created} {channel_type} channels created, {channels_not_created} channels not created - Total Time taken: {end_time_total - start_time_total:.2f} seconds""")
            ...
            log_message((Colorate.Horizontal(Colors.blue_to_purple,f"[-] There is no such server as you enter.")))
    except Exception as e:
        log_message((Colorate.Horizontal(Colors.blue_to_purple,f"[-]Please give the bot administrator permissions {e}"))) 
